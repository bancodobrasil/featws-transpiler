...
Inside it, create the files parameters.json, features.json, and rules.featws. If you need a group, create a new `groups` folder as in the steps described above. After all the essential files are filled in, we recommend creating the `expected.grl` file manually so you compare it with the rules file generated by the transpiler. 
